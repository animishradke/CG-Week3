...
public class FoodOrderSystem {
    public static void main(String[] args) {
        ...
        for (FoodItem item : orderItems) {
            ...
            System.out.println("Total price for this item: " + String.format("%.2f", itemTotalPrice));
            System.out.println("--------------------");
        }

        System.out.println("Total order price before discount: " + String.format("%.2f", totalOrderPrice));

        Discountable bulkDiscount = new BulkOrderDiscount();
        int totalQuantity = orderItems.stream().mapToInt(FoodItem::getQuantity).sum();
        if (totalQuantity > ((BulkOrderDiscount) bulkDiscount).bulkQuantityThreshold) {
            totalOrderPrice = bulkDiscount.applyDiscount(totalOrderPrice);
            System.out.println(bulkDiscount.getDiscountDetails());
        }

        System.out.println("Final total order price: " + String.format("%.2f", totalOrderPrice));
    }
}